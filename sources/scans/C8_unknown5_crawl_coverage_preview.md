# C8 未知 #5 预扫：analysis/crawl 覆盖（ForgeCrawl / RagRabbit / Crawl2RAG）
...
2. **P2 映射**：三者当前均在 analysis/crawl 中列为抓取+RAG 或 AI 问答类；若纳入，建议 P2 触发与现有 Crawl4AI/Firecrawl 等一致（两处独立采纳信号 + 风险与回滚说明），并注明「证据稀疏、待外部验证」。  
3. **内部归纳**：当前 crawl 结论主要来自 analysis/crawl 文档与 C8 A/B/C 产出；对 ForgeCrawl/RagRabbit/Crawl2RAG 的「采纳/风险」若延后纳入，可在 adoption_backlog 或 digest 中保留一句「内部归纳、待下轮外部证据补全」。
